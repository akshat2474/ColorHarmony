...
class BattleBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    paint.shader = ui.Gradient.radial(
      sunCenter,
      size.width * 0.13,
      [sunColor.withValues(alpha:0.8), sunColor.withValues(alpha:0.0)],
    );
    ...
    for (int i = 0; i < 8; i++) {
      ...
      final rayPaint = Paint()
        ..color = sunColor.withValues(alpha:0.13)
        ..strokeWidth = 5;
  ...
  void _drawPlatform(Canvas canvas, Size size, Alignment alignment) {
    ...
    paint.color = platformHighlightColor.withValues(alpha:0.35);
    canvas.drawArc(platformRect.deflate(12.0), pi * 1.1, pi * 0.5, false, paint);
    paint.color = platformShadowColor.withValues(alpha:0.15);
    paint.strokeWidth = 2;
